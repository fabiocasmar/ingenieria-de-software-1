...
class TestTasaEstacionamiento(TestCase):
    ...
    def test_estacionamiento_reserva_una_hora_sin_cambio_fecha(self): # Normal TDD
        ...
        Reserva(estacionamiento= e,inicioReserva=fecha_inicio,finalReserva=fecha_fin, tipo_puesto = 'Particular').save()
        self.assertEqual(tasa_reservaciones(e.id),salida)
    ...
    def test_estacionamiento_reserva_una_hora_dos_puestos(self): # Borde
        ...
        lista_valores[0]=90
        salida=dict(zip(lista_fechas,lista_valores))
        Reserva(estacionamiento= e,inicioReserva=ahora,finalReserva=ahora+timedelta(seconds=2700), tipo_puesto = 'Particular').save()
        Reserva(estacionamiento= e,inicioReserva=ahora+timedelta(seconds=2700),finalReserva=ahora+timedelta(seconds=5400), tipo_puesto = 'Particular').save()
        self.assertEqual(tasa_reservaciones(e.id),salida)
    ...
    def test_dos_reservaciones_mismo_dia(self): # Borde
        ...
        salida=dict(zip(lista_fechas,lista_valores))
        Reserva(estacionamiento= e,inicioReserva=ahora,finalReserva=ahora+timedelta(1), tipo_puesto = 'Particular').save()
        Reserva(estacionamiento= e,inicioReserva=ahora,finalReserva=ahora+timedelta(1), tipo_puesto = 'Particular').save()
        self.assertEqual(tasa_reservaciones(e.id),salida)
    ...
    def test_reserva_6_dias_misma_hora(self): # Normal TDD
        ...
        salida=dict(zip(lista_fechas,lista_valores))
        Reserva(estacionamiento= e,inicioReserva=ahora.replace(hour=6)+timedelta(2),finalReserva=ahora.replace(hour=6)+timedelta(6), tipo_puesto = 'Particular').save()
        x=tasa_reservaciones(e.id)
        self.assertEqual(tasa_reservaciones(e.id),salida)
    ...
    def test_reservaciones_de_una_hora_24_horas(self): # Esquina
        ...
        for i in range(CAPACIDAD):
            ...
            for j in range(HORAS_SEMANA):
                if j!=HORAS_SEMANA-1:
                    Reserva(estacionamiento=e,inicioReserva=hora_reserva,finalReserva=hora_reserva+UNA_HORA, tipo_puesto = 'Particular').save()
                else:
                    Reserva(estacionamiento=e,inicioReserva=hora_reserva,finalReserva=hora_reserva+timedelta(minutes=59), tipo_puesto = 'Particular').save()
